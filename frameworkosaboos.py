# -*- coding: utf-8 -*-
"""frameworkOSaboOs.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1I96A5vwKlBeeJVlJngazPUIb2ZMaWKIo
"""
...
class model():
    ...
    def fit(self, x_train, y_train, epochs=0, validation_split=0.1):
        ...
        for i in range(epochs):
            ...
            splitor=int(samples*validation_split)
            ...
            for j in range(1000):
                ...
                grad=self.delta_cross_entropy(self,output_layer[-1],y_train[j])

               # loss += self.loss( output_layer[j],y_train[j])

                   
                backward_input =  output_layer[-1]    
                ...
                back=len(output_layer)-2    #34an f el back propagation 3ayz el layer ely abl el 25era tb2a input w na2st wa7ed 34an el index bta3 el python bybd2 mn 1 m4 0
                for layer in  reversed_layers:
                    
                   
                    if (layer=="flatten"):
                     
                      backward_output=backward_output.reshape(flatten_shape)     #el arkam dy hatt3del dy mo2tka 
                      grad=backward_output   
                      back=back-1

                    else:
                      if (layer==reversed_layers[-1]):
                        
                        backward_output=layer.backward(grad, data)
                        grad=backward_output
                        
                      else:
                        
                        backward_output=layer.backward(grad,output_layer[back])
                        grad=backward_output
                        back=back-1
            ...
            for l in range(1000):  
                ...
                validation_grad+=self.delta_cross_entropy(self,validation_forward_input,y_validation[l])
           ...
           # validation_loss += self.loss(forward_input,y_train[l])
            ...
            print("Loss ={}      and      accuracy={}".format(self.loss (train_predection, y_train[0:1000]),self.accuracy(forward_outputs,y_train[0:1000])))
            print("validation_loss ={}     and      validation_accuracy={}".format( self.loss (validation_predection, y_validation[0:1000]),self.accuracy(validation_forward_outputs,y_validation[0:1000])))
...
model_test.fit(train_images , train_labels,epochs=3,validation_split=0.1)

C1=Conv(filters=6,n_prev=1,kernel_size=5, strides=1, padding="same",activation="tanh",learning_rate=0.1)
...
for i in range(0,count):
  ...
  convout=C1.forward(data)
  poolout= S2.forward(convout)
  ...
  a=np.argmax(denseout)
  r.append(a)
  k.append(train_labels[i])
  graddense=FC6.backward(grad,poolflat)
  graddense=graddense.reshape((14,14,6))
  gradpool= S2.backward(graddense,convout)
